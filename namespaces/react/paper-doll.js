namespace("gizmo-atheneum.namespaces.react.PaperDoll", {}, () => {
  return function ({ dataset, schematic, getLayerLabel, callback }) {
    ...
    const { dim, defs, background, svgLayers, frameWidth, frameHeight } = dataset.buildSVGComponents(schematic);

    return (<svg width={ frameWidth } height={ frameHeight } viewBox={ dim.join(" ") }>
